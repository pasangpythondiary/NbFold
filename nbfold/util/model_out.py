...
from nbfold.util.get_bins import get_bin_values
from nbfold.util.masking import MASK_VALUE
from nbfold.util.util import load_full_seq, one_hot_seq, get_heavy_seq_len
from nbfold.models.ProtT5.ProtT5_get_embedding import * 
...
def get_logits_from_model(model, fasta_file, device=None):
    ...
    if not device == None:
        seq = seq.to(device)
    emb=get_protT5_emb(fasta_file).to(device)
