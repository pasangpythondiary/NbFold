...
from nbfold.build_fv.build_cen_fa import build_initial_fv, get_cst_defs, refine_fv
...
def build_structure(model,
    ...
    out_pdb = os.path.join("nano/", "{}.pdb".format(filename))
...
def _get_args():
    ...
    default_model_dir = "trained_models/ensemble"
    parser.add_argument(
        "--model_dir",
        type=str,
...
def _cli():
    ...
    device_type = 'cuda:4' if torch.cuda.is_available(
    ...
    #creating folder
    # pred_all= os.path.join(out_dir, "pred_all")
    os.makedirs("nano", exist_ok=True)
    # Get a list of all fasta files in a directory
